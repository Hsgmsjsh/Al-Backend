...
async def root():
    ...
    try:
        ...
        logger.info(f"✅ Health check requested - {videos_count} videos in database")
        return response
    except Exception as e:
...
async def list_videos(limit: int = 50, skip: int = 0):
    """List all indexed videos with pagination."""
    try:
        ...
        async for doc in cursor:
            try:
                ...
                videos.append(video_out)
            except Exception as e:
                logger.warning(f"⚠️ Skipping malformed video document {doc.get('_id')}: {e}")
                continue
